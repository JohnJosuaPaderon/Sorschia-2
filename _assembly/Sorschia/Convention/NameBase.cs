...
namespace Sorschia.Convention
{
    public abstract class NameBase
    {
        ...
        private static T TryGet<T>(T backingField, string propertyName)
        {
            ...
            {
                throw new InvalidOperationException($"Sorschia.Convention.NameBase.{propertyName} was not set.");
            }
            else
            {
                return backingField;
        ...
        private static void TrySet<T>(ref T backingField, T value, string propertyName)
        {
            ...
            {
                throw new InvalidOperationException($"Sorschia.Convention.NameBase.{propertyName} cannot be set to its default value.");
            }
            else
            {
                backingField = value;
